...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
	...
	private Boolean setSelectedDateByCoor(float x, float y) {
		// cell click down
		if (y > surface.monthHeight + surface.weekHeight) {
			...
			System.out.println("downDate=="+downDate.getTime());
			System.out.println("当前时间=="+System.currentTimeMillis());
		}
		Boolean b;
		if(downDate.getTime()<System.currentTimeMillis()){
			b=false;
		}else{
			invalidate();//仅当几天以后的时间可以选择,刷新界面
			b=true;
		}
		return b;
//		invalidate();
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			if(setSelectedDateByCoor(event.getX(), event.getY())){
				
			}else{
				return false;//选择了今天以前的时间的时候就不再往下执行
			}
			break;
		case MotionEvent.ACTION_UP:
			if (downDate != null) {
				if(isSelectMore){
					...
					selectedStartDate = selectedEndDate = downDate;
					if( downDate.getTime() < System.currentTimeMillis() ){
						Toast.makeText(getContext(), "不能选择这个日期", 0).show();
						System.out.println("时间选择错误");
						return true;
					}else{
						//响应监听事件
						onItemClickListener.OnItemClick(selectedStartDate,selectedEndDate,downDate);
	...
	private class Surface {
		...
		public int bgColor = Color.parseColor("#FFFFFF");//背景的颜色
		...
		private int borderColor = Color.parseColor("#FFFFFF");//边框的颜色
		public int todayNumberColor =Color.parseColor("#FAC217");//当前日期字体颜色
		public int cellDownColor = Color.parseColor("#07CEC9");//点击后的颜色
		public int cellSelectedColor = Color.parseColor("#07CEC9");//选中的颜色
